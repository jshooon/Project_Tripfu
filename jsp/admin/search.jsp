...
<div class="flex">
			<c:if test="${not empty pproduct || not empty fproduct }">
				<c:forEach var = "p" items="${pproduct}">
			...
			<c:forEach var = "f" items="${fproduct}">
				<div class="col-lg-3 col-md-6">
					<div class="card" style="width: 18rem;">
			...
			<c:forEach var = "pi" items="${pInfo}">
				<div class="col-lg-3 col-md-6">
					<div class="card" style="width: 18rem;">
						<a href = "/tripfu/pc/pdetail/${pi.pnum}">
						  <img src="/images/${pi.ifname}" class="card-img-top" alt="...">
						</a>
					  <div class="card-body">
					    <h5 class="card-title">${pi.ptitle}</h5> 
					    <label class ="ptype">${pi.type} 여행</label>
					    <p class="card-text">${pi.pcontents}</p>
				  	  </div>
				  	  	<c:if test="${not empty sessionScope.id }">
				  	  		<a href="/tripfu/admin/pedit?pnum=${p.pnum}" id = "fbtn"class="btn btn-outline-success">수정하기</a>
						    <a href="javascript:pdeleted(${pi.pnum});" id = "pbtn" class="btn btn-outline-danger">삭제하기</a>			  	  	
				  	  	</c:if>
					    <a href="/tripfu/pc/pdetail/${pi.pnum}" class="btn btn-outline-primary">예약하기</a>
					</div>
				</div>
			</c:forEach>
			<c:forEach var = "fi" items="${fInfo}">
				<div class="col-lg-3 col-md-6">
					<div class="card" style="width: 18rem;">
						<a href = "/tripfu/pc/fdetail/${fi.pnum}">
						  <img src="/images/${fi.ifname}" class="card-img-top" alt="...">
						</a>
					  <div class="card-body">
					    <h5 class="card-title">${fi.ptitle}</h5> 
					    <label class ="ptype">${fi.type} 여행</label>
					    <p class="card-text">${fi.pcontents}</p>
				  	  </div>
				  	  	<c:if test="${not empty sessionScope.id }">
				  	  		<a href="/tripfu/admin/fedit?pnum=${f.pnum}" id = "fbtn"class="btn btn-outline-success">수정하기</a>
						    <a href="javascript:fdeleted(${fi.pnum});" id = "fbtn"class="btn btn-outline-danger">삭제하기</a>			  	  	
				  	  	</c:if>
					    <a href="/tripfu/pc/fdetail/${fi.pnum}" class="btn btn-outline-primary">예약하기</a>
					</div>
				</div>
			</c:forEach>			
</div>
