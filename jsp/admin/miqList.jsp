...
<meta charset="utf-8">
<title>Tripfu [회원문의내역 목록]</title>
<link rel="stylesheet" type="text/css" href="/css/admin/miqList.css">
</head>
<body>
				 ...
				 <c:forEach var = "i" items="${pageInfo.list}">
				    <tr>
				      ...
				      <td><button type="button" class="btn btn-outline-secondary" 
				      			onclick = "location.href='/tripfu/admin/miqDetail?num=${i.num}'">문의상세내용</button></td>
				    </tr>
				 </c:forEach>
  			</tbody>
  	<!-- 테이블 바디 끝 -->
	...
	<!-- 페이징 시작 -->				
		<nav aria-label="Page navigation example">
		  <ul class="pagination justify-content-center">
		    <li class="page-item">
		      <a class="page-link" href = "/tripfu/admin/miqList/${pageInfo.navigateFirstPage}" aria-label="Previous">
			    ...
			    <c:forEach var = "pageNum" items = "${pageInfo.navigatepageNums}">
			    	<c:choose>									
						<c:when test = "${pageInfo.pageNum == pageNum}"> <!-- n이 현재 페이지와 같다면, -->
			    			<li class="page-item active"><a class="page-link" href = "/tripfu/admin/miqList/${pageNum}">${pageNum}</a></li>
			    		</c:when>
			    		<c:otherwise>
			    		<li class="page-item"><a class="page-link" href = "/tripfu/admin/miqList/${pageNum}">${pageNum}</a></li>
			    		</c:otherwise>
			    	</c:choose>		
			    </c:forEach>
			   <li class="page-item">
			    <a class="page-link" href = "/tripfu/admin/miqList/${pageInfo.navigateLastPage}"  aria-label="Next">
