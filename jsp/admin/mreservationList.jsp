...
<meta charset="utf-8">
<title>Tripfu [회원예약목록]</title>
<link rel="stylesheet" type="text/css" href="/css/admin/mreservationList.css">
</head>
<body>
<!-- 네비바 시작 -->
...
<table class="table">
  ...
  <tbody>
	 <c:forEach var = "l" items="${rlist.list}">
	    <tr>
	      <td>${l.rtype }</td>
	      <td><img src="/images/${l.rpname}.jpg" class="rounded mx-auto d-block" width = "100px;" height = 100px;></td>
	      <td>${l.rname}</td>
	      <td>${l.rpname}</td>
	      <td>${l.sdate} ~ ${l.edate}</td>
	      <td><button type="button" class="btn btn-outline-secondary" 
	      			onclick = "location.href='/tripfu/admin/mrDetail?rnum=${l.rnum}'">상세보기</button></td>
	    </tr>
	 </c:forEach>
  </tbody>
</table>
	...
	<!-- 페이징 시작 -->				
		<nav aria-label="Page navigation example">
		  <ul class="pagination justify-content-center">
		    <li class="page-item">
		      <a class="page-link" href = "/tripfu/admin/mreservationList/${rlist.navigateFirstPage}" aria-label="Previous">
		        <span aria-hidden="true">&laquo;</span>
		      </a>
		    </li>
			    <c:forEach var = "pageNum" items = "${rlist.navigatepageNums}">
			    	<c:choose>									
						<c:when test = "${rlist.pageNum == pageNum}"> <!-- n이 현재 페이지와 같다면, -->
			    			<li class="page-item active"><a class="page-link" href = "/tripfu/admin/mreservationList/${pageNum}">${pageNum}</a></li>
			    		</c:when>
			    		<c:otherwise>
			    		<li class="page-item"><a class="page-link" href = "/tripfu/admin/mreservationList/${pageNum}">${pageNum}</a></li>
			    		</c:otherwise>
			    	</c:choose>		
			    </c:forEach>
			   <li class="page-item">
			    <a class="page-link" href = "/tripfu/admin/mreservationList/${rlist.navigateLastPage}"  aria-label="Next">
