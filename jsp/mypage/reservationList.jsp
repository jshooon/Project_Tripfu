	 ...
	 <c:forEach var = "l" items="${rList.list}">
	    <tr>
	      ...
	      <td><button type="button" class="btn btn-outline-secondary" 
	      			onclick = "location.href='/tripfu/mypage/rDetail?rnum=${l.rnum}'">상세보기</button></td>
	    </tr>
	 </c:forEach>
  </tbody>
<!-- 테이블 바디 끝 --> 
	...
	<!-- 페이징 시작 -->
		<nav aria-label="Page navigation example">
		  <ul class="pagination justify-content-center">
		    <li class="page-item">
		      <a class="page-link" href = "/tripfu/mypage/reservationList/${rList.navigateFirstPage}?mnum=${mnum}" aria-label="Previous">
			    ...
			    <c:forEach var = "pageNum" items = "${rList.navigatepageNums}">
			    	<c:choose>									
						<c:when test = "${rList.pageNum == pageNum}"> <!-- n이 현재 페이지와 같다면, -->
			    			<li class="page-item active"><a class="page-link" href = "/tripfu/mypage/reservationList/${pageNum}?mnum=${mnum}">${pageNum}</a></li>
			    		</c:when>
			    		<c:otherwise>
			    		<li class="page-item"><a class="page-link" href = "/tripfu/mypage/reservationList/${pageNum}?mnum=${mnum}">${pageNum}</a></li>
			    		</c:otherwise>
			    	</c:choose>		
			    </c:forEach>
			   <li class="page-item">
			    <a class="page-link" href = "/tripfu/mypage/reservationList/${rList.navigateLastPage}?mnum=${mnum}"  aria-label="Next">
